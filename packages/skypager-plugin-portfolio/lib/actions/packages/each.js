...
execute(function (group) {
  ...
  var paths = include.map(function (a) {
    return a.paths.absolute;
  });
  ...
  if (cmd === 'npm' && args[0] === 'run') {
    include = include.filter(function (pkg) {
      return args[1] && pkg.data && pkg.data.scripts && pkg.data.scripts[args[1]];
    });
  }
